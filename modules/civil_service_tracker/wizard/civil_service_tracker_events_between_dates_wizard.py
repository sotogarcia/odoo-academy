...
class CivilServiceTrackerEventsBetweenDatesWizard(models.TransientModel):
    ...
    _description = "Civil service tracker events between dates wizard"
    ...
    date_type = fields.Selection(
        ...
        default="event_date",
        help=(
            "Determines which date field is used to filter events: event "
    ...
    date_start_tz = fields.Datetime(
        ...
        default=None,
        help=(
            "start of the period to collect and notify events "
    ...
    date_stop_tz = fields.Datetime(
        ...
        default=None,
        help=(
            "End of the period to collect and notify events "
            "(in the selected time zone)."
        ),
    ...
    sender_type = fields.Selection(
        ...
        help=(
            "Defines the type of sender: a company, a specific user, or a "
    ...
    reply_to_email = fields.Char(
        ...
        help=(
            "Email address used for replies. Defaults to a company-specific "
            "no-reply address."
        ),
        translate=False,
    )
    ...
    can_choose_sender = fields.Boolean(
        ...
        help=(
            "Indicates whether the current user can choose another user "
            "as sender."
        ),
    )
    ...
    process_event_ids = fields.Many2many(
        ...
        default=None,
        help=(
            "List of events that occurred between the selected dates and "
    ...
    def create(self, values_list):
        ...
        for values in values_list:
            self._check_sender_access(values.get("sender_id"))
    ...
    def _is_all_day(cls, date_start, date_stop):
        return (
            date_start.time() == time.min
            and date_stop.time() == time.min
            and (date_stop - date_start) == timedelta(days=1)
        )
    ...
    def perform_action(self):
        ...
        for recipient in self.recipient_ids:
            ...
            context.update(
                grouped_events=grouped, recipient=recipient, base_url=base_url
            )
