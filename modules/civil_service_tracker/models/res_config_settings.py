...
class ResConfigSettings(models.TransientModel):
    ...
    tracking_responsible_id = fields.Many2one(
        ...
        help=('Person responsible for supervising civil service selection ' 
              'processes'),
    ...
    erp_manager_id = fields.Many2one(
        ...
        index=False,
        default=lambda self: self.env.ref('base.user_admin'),
        help=('Person in charge of maintaining the civil service tracker '
              'module'),
        comodel_name='res.users',
        domain=[],
        context={},
        ...
        if menu and menu.action:
            if isinstance(menu.action, str):
                ...
                if model == 'ir.actions.act_window':
                    act_window = self.env['ir.actions.act_window'].browse(int(action_id))
