...
class CivilServiceTrackerSelectionProcess(models.Model):
    ...
    _description = u'Civil service tracker selection process'
    ...
    def _onchange_public_offer_id(self):
        for record in self:
            ...
            if new_record or not record.image_1920:
                ...
                admin_image = (
                    record.public_offer_id
                    .public_administration_id.image_1920
                )
    ...
    annual_payments = fields.Integer(
        ...
        index=False,
        default=14,
        help=('Number of salary payments per year, including extra payments '
              'if any'),
        track_visibility='onchange'
    )
    ...
    def _compute_process_infographics_count(self):
        for record in self:
            record.process_infographics_count = len(record.process_infographics_ids)
    ...
    def _compute_syllabus_breakdown_count(self):
        for record in self:
            record.syllabus_breakdown_count = len(record.syllabus_breakdown_ids)
    ...
    @api.depends('process_infographics_count', 'syllabus_breakdown_count', 
                 'exam_infographics_count', 'student_results_count',
    ...
    aggregated_attachment_ids = fields.Many2many(
        ...
        index=False,
        default=None,
        help=('Combined list of all documents attached to this selection '
    ...
    @api.depends('process_infographics_ids', 'syllabus_breakdown_ids', 
                 'exam_infographics_ids', 'student_results_ids', 
    ...
    @api.constrains('service_position_id', 'employment_group_id', 
                    'public_administration_id')
    def _check_service_position_consistency(self):
        # Mensaje base para el error
        message = _("The '{fs}' of the selected Service Position must match "
    ...
    def _attach_approval_event_journal_url(self, event, journal_url):
        ...
        if attachment:
            ...
            event.attachment_id = self.env['ir.attachment'].create(attachment_values)
