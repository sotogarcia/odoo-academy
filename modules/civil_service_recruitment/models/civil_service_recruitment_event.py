...
class CivilServiceRecruitmentEvent(models.Model):
    """ Each of the state changes in a civil service recruitment process
    """

    _name = 'civil.service.recruitment.event'
    ...
    def _onchange_event_type_id(self):
        type_obj = self.env['civil.service.recruitment.event.type']
    ...
    def _ensure_name(self, value_dict, type_id=False):
        ...
        if not value_dict.get('name', False):
            type_model = 'civil.service.recruitment.event.type'
    ...
    def create(self, values):
        ...
        result = super(CivilServiceRecruitmentEvent, self).create(values)
    ...
    def write(self, values):
        ...
        result = super(CivilServiceRecruitmentEvent, self).write(values)

        # STEP 3: Update state and dates in the related processes
        model = self.env.context.get('model', False)
        if model != 'civil.service.recruitment.process':
            self.process_id.touch()
    ...
    def notify_creation(self):
        ...
        for record in self:
            xid = 'civil_service_recruitment_event_created'
            subtype = record.env.ref('civil_service_recruitment.' + xid)
    ...
    def notify_update(self):
        ...
        for record in self:
            xid = 'civil_service_recruitment_event_written'
            subtype = record.env.ref('civil_service_recruitment.' + xid)
            process = record.process_id
