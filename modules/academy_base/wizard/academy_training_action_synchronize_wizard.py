...
class AcademyTrainingActionSynchronizeWizard(models.TransientModel):
    ...
    sync_details = fields.Boolean(
        string="Sync Action Details",
        required=False,
        readonly=False,
        index=False,
        default=True,
        help="Also synchronize the detailed information of the training "
        "action during synchronization",
    )
    ...
    def synchronize_training_groups(self, **kwargs):
        ...
        unlinked_action_set = self._stg_unlink(
            parent_line_set,
            child_line_set,
            remove_mismatches,
        )
    ...
    def _stg_unlink(parent_lines, child_lines, remove_mismatches):
        ...
        Returns:
            academy.training.action: Training actions affected by removals.
        """
        action_obj = child_lines.env["academy.training.action"]
        ...
        for parent_line in parent_lines:
            ...
            if parent_line.program_line_id:
                row["prog_ids"].add(parent_line.program_line_id.id)
            elif parent_line.code:
                row["codes"].add(parent_line.code)
