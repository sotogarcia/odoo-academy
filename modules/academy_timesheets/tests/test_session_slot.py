...
class TestAcademyTestsSessionSlot(TransactionCase):
    ...
    def _move_session(
        ...
        date_start = session.date_start + timedelta(
            days, seconds, microseconds, milliseconds, minutes, hours, weeks
        )

        date_stop = session.date_stop + timedelta(
            days, seconds, microseconds, milliseconds, minutes, hours, weeks
        )
    ...
    def test_slot_is_out(self):
        ...
        # 10:00 - 10:30
        self.assertFalse(slot.is_out(sessions["i1"]), "test_is_out: Inside")

        # 10:30 - 11:30
        self.assertFalse(slot.is_out(sessions["i2"]), "test_is_out: Inside")

        # 11:30 - 12:00
        self.assertFalse(slot.is_out(sessions["i3"]), "test_is_out: Inside")
    ...
    def test_slot_move(self):
        ...
        d = self.date_base.replace(hour=9)
        self.assertEqual(
            sessions["o1"].date_start, c, msg="test_slot_move: out session 1"
        )
        self.assertEqual(
            sessions["o1"].date_stop, d, msg="test_slot_move: out session 1"
        )
        ...
        f = self.date_base.replace(hour=10, minute=15)
        self.assertEqual(
            sessions["v1"].date_start,
            a + offset,
            msg="test_slot_move: out session 1",
        )
        self.assertEqual(
            sessions["v1"].date_stop,
            f + offset,
            msg="test_slot_move: out session 1",
        )
        ...
        h = self.date_base.replace(hour=11, minute=30)
        self.assertEqual(
            sessions["i2"].date_start,
            g + offset,
            msg="test_slot_move: out session 1",
        )
        self.assertEqual(
            sessions["i2"].date_stop,
            h + offset,
            msg="test_slot_move: out session 1",
        )
        ...
        # Session will be resized
        self.assertEqual(
            sessions["v2"].date_start,
            i + offset,
            msg="test_slot_move: out session 1",
        )
        self.assertEqual(
            sessions["v2"].date_stop,
            b + offset,
            msg="test_slot_move: out session 1",
        )
        ...
        ll = self.date_base.replace(hour=14)
        self.assertEqual(
            sessions["o2"].date_start, k, msg="test_slot_move: out session 1"
        )
        self.assertEqual(
            sessions["o2"].date_stop, ll, msg="test_slot_move: out session 1"
        )

        # 9:00 - 10:00
        self.assertEqual(
            sessions["a1"].date_start, d, msg="test_slot_move: out session 1"
        )
        self.assertEqual(
            sessions["a1"].date_stop, a, msg="test_slot_move: out session 1"
        )

        # 10:00 - 10:30
        self.assertEqual(
            sessions["i1"].date_start,
            a + offset,
            msg="test_slot_move: out session 1",
        )
        self.assertEqual(
            sessions["i1"].date_stop,
            g + offset,
            msg="test_slot_move: out session 1",
        )
        ...
        # 11:30 - 12:00
        self.assertEqual(
            sessions["i3"].date_start,
            h + offset,
            msg="test_slot_move: out session 1",
        )
        self.assertEqual(
            sessions["i3"].date_stop,
            b + offset,
            msg="test_slot_move: out session 1",
        )
        ...
        # 12:00 - 13:00
        self.assertEqual(
            sessions["a2"].date_start, b, msg="test_slot_move: out session 1"
        )
        self.assertEqual(
            sessions["a2"].date_stop, k, msg="test_slot_move: out session 1"
        )
