...
class RedirectTo(Controller):
    ...
    def _tree_to_list(view_modes):
        ...
        if 'tree' in view_modes:
            view_modes[view_modes.index('tree')] = 'list'
    ...
    def _act_window_to_url(self, action, view_type='list'):
        ...
        assert view_type in view_modes, \
            _('Invalid view type for action «%s»' % action.name)
    ...
    def teachers_url(self, **kw):
        action_xid = ('academy_base.'
                      'action_teacher_act_window')
