...
class AcademyTimesheetsCloneWizard(models.TransientModel):
    ...
    def _compute_from_stop(self):
        for record in self:
            if record.interval_type == "day":
                ...
                record.from_start = self._next_month(record.from_start)
    ...
    def _compute_to_stop(self):
        for record in self:
            if record.interval_type == "day":
                ...
                record.to_stop = self._next_month(record.to_start)
    ...
    def _get_consistent_dates(self):
        ...
        if self.interval_type == "week":
            ...
            from_start = from_start.replace(day=1)
            to_start = to_start.replace(day=1)

            from_stop = self._next_month(from_start)
            to_stop = self._next_month(to_start)
        else:
    ...
    def _search_targets(self):
        ...
        if active_ids:
            ...
            msg = _("No records selected for which to clone sessions")
            _logger.info(msg)
    ...
    def perform_action(self):
        ...
        if self.are_dates_overlapped(from_start, from_stop, to_start, to_stop):
            msg = _("Source and destination ranges must not overlap")
    ...
    def _view_logs(self):
        ...
        name = _("Log history #{}").format(self.id)
    ...
    def _next_month(date_value):
        calculation_step = date_value.replace(day=1)
        calculation_step += relativedelta(months=1)
        calculation_step -= relativedelta(days=1)

        if isinstance(calculation_step, datetime):
            calculation_step = calculation_step.date()
            calculation_step = datetime.combine(calculation_step, time.min)
