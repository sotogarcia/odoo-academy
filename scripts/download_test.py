...
class App:
    ...
    def _argparse(self):
        ...
        description = "Import a files into Odoo ir.attachement records"

        parser = argparse.ArgumentParser(description)
        # parser.add_argument('file', metavar='file', type=str,
        #                     help=u'path of the resource file will be stored')

        msg = "Odoo server address will be used to connect"
        parser.add_argument(
            "-s",
            "--server",
            type=str,
        ...
        msg = "Odoo server port will be used to connect"
        parser.add_argument(
            "-n", "--port", type=int, dest="port", default=None, help=msg
        )

        msg = "use HTTPS to connect to Odoo server"
        parser.add_argument(
            "--https", dest="https", action="store_true", help=msg
        )

        msg = "user will be used to login in Odoo server"
        parser.add_argument(
            "-u", "--user", type=str, dest="user", default="admin", help=msg
        )

        msg = "password will be used to login in Odoo server"
        parser.add_argument(
            "-p",
            "--password",
            type=str,
        ...
        msg = "name of the database will be used to store the resource"
        parser.add_argument(
            "-d",
            "--database",
            type=str,
        ...
        msg = "test identifier"
        parser.add_argument(
            "-i", "--id", type=int, dest="id", default=0, help=msg
        )

        msg = "test title"
        parser.add_argument(
            "-t", "--tittle", type=str, dest="title", default=None, help=msg
        )

        msg = "available test report"
        # default=u'academy_tests.view_academy_tests_qweb',
        parser.add_argument(
            "-r", "--report", type=str, dest="report", help=msg
        )

        msg = "destination file"
        parser.add_argument(
            "-o", "--output", type=str, dest="output", default=None, help=msg
        )

        msg = "download all attachments"
        parser.add_argument(
            "-a",
            "--attachs",
            dest="attachments",
        ...
        msg = "create additional solutions table text file"
        parser.add_argument(
            "-l", "--table", dest="solutions", action="store_true", help=msg
        )

        msg = "show process information"
        parser.add_argument(
            "-v", "--verbose", dest="verbose", action="store_true", help=msg
        )
        ...
        if server.lower().startswith(("http://", "https://")):
            ...
            server_https = parsed_url.scheme.lower() == "https"
            server_port = parsed_url.port
        else:
            self._server = server

        self._https = args.https or server_https
        self._port = args.port or server_port or (443 if self._https else 8069)
    ...
    def _ensure_target(self, args):
        ...
        if args.id > 0:
            ...
            files = [item for item in os.listdir(".") if item.endswith(".ID")]
            if files:
                self._id = int(files[0][:-3])
                self._read_id_file(files[0])
                print("Using file " + files[0] + " and " + self._report)

        if args.title:
            self._title = args.title.decode(self._cp, errors="replace")
        ...
        with open(fname, "r") as finput:  # open the file
            ...
            for line_raw in lines:
                ...
                if re.match(r"^report\= *[^ ]+ *$", line, re.IGNORECASE):
                    self._report = line.replace("report=", "").strip()
    ...
    def _connect(self):
        ...
        try:
            protocol = "jsonrpc+ssl" if self._https else "jsonrpc"
            self._print(
                "Connecting to {}://{}:{}",
            ...
            _logger.exception("Hubo un problema al conectar.")
    ...
    def _login(self):
        ...
        try:
            ...
            _logger.exception("Hubo un problema al iniciar sesión.")
    ...
    def main(self):
        ...
        if (self._id or self._title) and self._connect():
            ...
            print("Please type an ID or a title, see --help")
